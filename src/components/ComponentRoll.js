...
class ComponentRollTemplate extends React.Component {
	render() {
...
export default function ComponentRoll() {
	return (
		<StaticQuery
			query={graphql`
				query ComponentRollQuery {
			...
			`}
			render={(data, count) => <ComponentRollTemplate data={data} count={count} />}
		/>
	);
}
